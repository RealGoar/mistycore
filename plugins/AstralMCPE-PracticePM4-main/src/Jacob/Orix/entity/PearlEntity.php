...
final class PearlEntity extends Throwable
{
	...
	protected function onHitEntity(Entity $entityHit, RayTraceResult $hitResult): void
	{
        if ($this->getOwningEntity() instanceof Player) {
            $this->getOwningEntity()->setPosition($hitResult->getHitVector());
            $this->getOwningEntity()->getNetworkSession()->syncMovement($this->getOwningEntity()->getLocation(), $this->getOwningEntity()->getLocation()->getYaw(), $this->getOwningEntity()->getLocation()->getPitch());
            $this->getOwningEntity()->attack(new EntityDamageEvent($this->getOwningEntity(), EntityDamageEvent::CAUSE_PROJECTILE, 2));
            $this->getWorld()->addSound($this->getOwningEntity()->getPosition(), new EndermanTeleportSound());

            if ($entityHit instanceof Player)
            $entityHit->attack(new EntityDamageByChildEntityEvent($this->getOwningEntity(), $this, $entityHit, EntityDamageEvent::CAUSE_PROJECTILE, 2));
        }
        $this->kill();
    }
		...
		if ($this->getOwningEntity()) {
			...
			if (in_array($blockHit->getId(), $blocksAllowed, true)) {
				...
				if ($blockBack->getId() === BlockLegacyIds::AIR && $blockBack->getPosition()->getWorld()->getBlock($blockBack->getPosition()->add(0, 1, 0))->getId() === BlockLegacyIds::AIR) {
					$this->getWorld()->addSound($this->getOwningEntity()->getPosition(), new EndermanTeleportSound());
					$this->getOwningEntity()->setPosition($blockBack->getPosition());
					$this->getOwningEntity()->getNetworkSession()->syncMovement($this->getOwningEntity()->getLocation(), $this->getOwningEntity()->getLocation()->getYaw(), $this->getOwningEntity()->getLocation()->getPitch());
					$this->kill();
				} else {
					$this->teleportAt();
	...
	protected function teleportAt(): void
	{
		...
		if ($this->getPosition()->getY() > 0) {
			if ($this->isPearling()) {
				...
				$this->getOwningEntity()->setPosition(Position::fromObject($this->getOwningEntity()->getPosition()->add($direction->x, (int)$direction->y + 1, $direction->z), $this->getOwningEntity()->getWorld()));
				$this->getOwningEntity()->getNetworkSession()->syncMovement($this->getOwningEntity()->getLocation(), $this->getOwningEntity()->getLocation()->getYaw(), $this->getOwningEntity()->getLocation()->getPitch());
                $this->getOwningEntity()->attack(new EntityDamageEvent($this->getOwningEntity(), EntityDamageEvent::CAUSE_PROJECTILE, 2));
